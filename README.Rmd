...
Landscape affects movement (e.g. terrain, land cover)
→ provide ```resistance_rast``` or ```dem_rast```

Uphill vs downhill matters (directional movement)
→ ```anisotropy = "terrain"```

Repeated runs (uncertainty or time series)
→ use ```prepare_geodesic_context() + geodesic_engine = "multisource"```
