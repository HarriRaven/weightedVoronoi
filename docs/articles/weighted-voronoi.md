...
Geodesic tessellations compute shortest-path distances constrained to
the domain.
...
resistance. Topography, land cover, and infrastructure can increase or
block movement.

The recommended workflow is to build a resistance surface (optionally
composed from multiple rasters), apply barriers, and then pass the
result to weighted_voronoi_domain() via resistance_rast.
