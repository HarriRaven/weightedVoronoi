...
pts \<- st_sample(boundary_sf, size = 8, type = “random”) points_sf \<-
st_sf( village = paste0(“V”, seq_along(pts)), population =
round(exp(rnorm(length(pts), log(300), 0.8))), geometry = pts, crs =
crs_use )
...
“population”, boundary_sf = boundary_sf, res = 10, weight_transform =
log10, distance = “euclidean”, clip_to_boundary = TRUE, verbose = FALSE
)
...
out_euc$summaryout_{g}eo$summary
